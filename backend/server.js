...
function normalizeCartItems(items = []) {
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    return {
      ...
      packSize: String(item?.packSize || size).trim(),
      ...
      paidLineTotal,
      payableLineTotal: paidLineTotal,
      displayAmount: paidLineTotal,
      originalAmount: Math.round(toNumber(item?.originalAmount || (mrp || price) * qty)),
